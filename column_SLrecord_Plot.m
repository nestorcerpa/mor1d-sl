...
COLUMN.phip_top    = idft(MODEL.SL.dfs)*MODEL.par.rhow/MODEL.par.rhom.*abs(MODEL.SL.sl)/(MODEL.par.H*1e3);
%%% clh
MODEL.SL.dfs.gamma = gamma.*MODEL.clh_top;
COLUMN.clp_top     = idft(MODEL.SL.dfs)*MODEL.par.rhow/MODEL.par.rhom.*abs(MODEL.SL.sl)/(MODEL.par.H*1e3);
%%% fh 
MODEL.SL.dfs.gamma = gamma.*MODEL.fh_top;
COLUMN.fp_top      = idft(MODEL.SL.dfs)*MODEL.par.rhow/MODEL.par.rhom.*abs(MODEL.SL.sl)/(MODEL.par.H*1e3);
%%% fch 
MODEL.SL.dfs.gamma = gamma.*MODEL.fch_top;
COLUMN.fcp_top     = idft(MODEL.SL.dfs)*MODEL.par.rhow/MODEL.par.rhom.*abs(MODEL.SL.sl)/(MODEL.par.H*1e3);
...
plot(F31,MODEL.SL.time,COLUMN.clp_top./(MODEL.MFields.c(end)/MODEL.par.D_vol)*100.,'color',[1 0 0],'linewidth',linew); 
